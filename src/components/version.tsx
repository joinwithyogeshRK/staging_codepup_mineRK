...
const VersionHistoryModal: React.FC<VersionHistoryModalProps> = ({
  ...
  const fetchVersions = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:3000'}/api/versions/${projectId}`
      );
  ...
  const handleRestoreVersion = async (versionNumber: number) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:3000'}/api/versions/${projectId}/restore`,
        {
          method: 'POST',
          headers: {
