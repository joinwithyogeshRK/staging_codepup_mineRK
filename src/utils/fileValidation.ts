...
export async function validateFile(file: File): Promise<boolean | string> {
  ...
  if (!allowedExtensions.includes(fileExtension)) {
    return "🐾 Arf! Unsupported file type — our pup only fetches images, PDFs, and docs (XLSX/XLS/TXT/MD/CSV).";
  }
    ...
    return new Promise((resolve) => {
      reader.onload = function (e) {
        try {
          ...
          if (pageCount > 5) {
            resolve(
              "🐾 Arf! Too many pages — our pup can only fetch up to 5 pages per PDF."
            );
          } else {
            resolve(true);
          }
...
export async function validateModificationLimits(
  ...
  const existingPdfs = existingRawFiles.filter(
  ...
  const existingRawSize = existingRawFiles.reduce(
